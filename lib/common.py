...
class RewardTracker:
    ...
    def reward(self, reward_steps, frame, epsilon=None):
        #print("In reward function. Printing Reward and Steps")
        reward, steps = reward_steps
        self.reward_buf.append(reward)
        self.steps_buf.append(steps)
        #print(reward)
        #print(steps)
        #print(len(self.reward_buf))
        if len(self.reward_buf) < self.group_rewards:
            #print("returning 1st false in reward func")
            return False
        #print("After 1st return false in reward func")
        ...
        #print(self.stop_reward)
        #if mean_reward > self.stop_reward:
            #print("Solved in %d frames!" % frame)
            #return True
        if round(epsilon,2) == 0.10:
            print("Solved in %d frames!" % frame)
            return True
        return False
...
def calc_loss_doubleDQN(batch, net, tgt_net, gamma, device="cpu", double=True):
    states, actions, rewards, dones, next_states = unpack_batch(batch)
