...
# import environ, data, models, common, validation
...
DEFAULT_STOCKS = "/content/drive/Shared drives/Reinforcement Learning/Sithara/data/YNDX_160101_161231.csv"
DEFAULT_VAL_STOCKS = "/content/drive/Shared drives/Reinforcement Learning/Sithara/data/YNDX_150101_151231.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gamma", type=float, help="Gamma, overrides --gamma option", default=GAMMA)
    parser.add_argument("--learningrate", type=float, help="LEARNING_RATE, overrides --learningrate option",
    ...
    print(LEARNING_RATE)
    print(args.double)
    runpath = "/content/drive/Shared drives/Reinforcement Learning/Sithara/saves"
    print(runpath)
    print(args.run)
    saves_path = runpath + args.run
    # saves_path = os.path.join(runpath, args.run)
    print(saves_path)
    os.makedirs(saves_path, exist_ok=True)
    print("here")
    print(args.data)
    # data = 'data/YNDX_160101_161231.csv'
    file_exists = tf.io.gfile.exists(args.data)

    if args.year is not None or file_exists:
    ...
    writer = SummaryWriter(logdir="/content/drive/Shared drives/Reinforcement Learning/Sithara/runs/" + args.run,
                           comment="-double=" + args.run)
    ...
    with common.RewardTracker(writer, np.inf, group_rewards=100) as reward_tracker:
        while True:
            ...
            #print("New Rewards")
            if new_rewards:
                #print("If new rewards")
                #print(new_rewards[0])
                if reward_tracker.reward(new_rewards[0], step_idx, selector.epsilon):
                    break
            ...
            loss_v = common.calc_loss_doubleDQN(batch, net, tgt_net.target_model, GAMMA ** REWARD_STEPS, device=device, double=args.double)
            loss_v.backward()
            optimizer.step()
